...
//		System.out.println(collect);
		 ...
		 * long count2 = empData.stream().filter(e ->
		 * e.gender.equals("Female")).count();
		 * 
		 ...
		 * empData.stream().collect(Collectors.groupingBy(Employee::getGender,Collectors
		 * .counting())); System.out.println(collect);
		 * 
		 * Q. print the name of all departments in the organization ?
		 * 
		 * =>empData.stream() .map(e->e.depertmanet) .distinct()
		 * .forEach(i->System.out.println(i));
		 * 
		 * Q.Avg age of male and female employee ? => Map<String,Double> collect =
		 * empData.stream()
		 * .collect(Collectors.groupingBy(Employee::getGender,Collectors.averagingInt(
		 * Employee::getAge))); System.out.println(collect);
		 * 
		 * Q. Get the details highest paid employee in organization ? =>
		 * Optional<Employee> collect =
		 * empData.stream().collect(Collectors.maxBy(Comparator.comparing(e->e.salary)))
		 * ; Employee employee = collect.get();
		 * System.out.println("Max salery :"+employee);
		 * 
		 * Q.Get the names of all employee who joined after 2015 => List<Employee>
		 * collect2 = empData.stream().filter(e->e.yearOfjoining >
		 * 2015).collect(Collectors.toList()); collect2.forEach(name->
		 ...
		 * .collect(Collectors.groupingBy(Employee::getDepertmanet,Collectors.
		 * averagingDouble(Employee::getSalary))); System.out.println(collect);
		 * 
		 * Q. Get the details of youngest male employee in the product development
		 * department ?
		 * 
		 * =>Optional<Employee> min = empData.stream() .filter(e ->
		 * e.gender.equals("male") && e.depertmanet.equals("Developement"))
		 * .min(Comparator.comparing(e->e.age)); if(min.isPresent()) { Employee employee
		 * = min.get(); System.out.println(employee); }
		 * 
		 ...
		 * if(collect.isPresent()) { Employee employee = collect.get();
		 ...
		 * .collect(Collectors.groupingBy(Employee::getDepertmanet,Collectors.groupingBy
		 * (Employee::getName))); System.out.println(collect+"/t");
		 * 
		 * Q. Sum of all the salaries ?
		 * 
		 * => DoubleSummaryStatistics collect = empData.stream()
		 * .collect(Collectors.summarizingDouble(Employee::getSalary)); double sum =
		 * collect.getSum(); System.out.println(sum);
		 * 
		 * Q. Second heights salary ?
		 * 
		 * => List<Employee> collect =
		 * empData.stream().sorted(Comparator.comparing(Employee::getSalary).reversed())
		 * .collect(Collectors.toList()); Employee employee = collect.get(1);
		 * System.out.println(employee);
		 * 
		 * //Another way
		 * 
		 * Optional<EmployeeData> first =
		 * empData.stream().sorted(Comparator.comparing(EmployeeData::getSalary).
